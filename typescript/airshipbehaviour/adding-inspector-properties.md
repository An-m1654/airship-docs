...
### Optionally showing properties

You may want to only show certain properties only \*if\* a condition is met with other properties.

This can be done with `ShowIf` and `HideIf`.

```typescript
export default class ExampleAirshipBehaviour extends AirshipBehaviour {
	public toggleSetting = false;

	@ShowIf("toggleSetting")
	public valueShowOnToggle = "Hello, world!";
}
```

<figure><img src="../../.gitbook/assets/image.png" alt=""><figcaption><p>Property not shown if <code>toggleSetting</code> is false</p></figcaption></figure>

<figure><img src="../../.gitbook/assets/image (1).png" alt=""><figcaption><p>Property shown if <code>toggleSetting</code> is true</p></figcaption></figure>

You can also use the _match_ versions of these decorators to show values on certain values, such as with enums:

```typescript
export enum ExampleTargetType {
	Position,
	Transform,
}

export default class ExampleAirshipBehaviour extends AirshipBehaviour {
	public targetType: ExampleTargetType;

	@ShowIf("targetType", ExampleTargetType.Position)
	public targetPosition: Vector3;

	@ShowIf("targetType", ExampleTargetType.Transform)
	public targetTransform: Transform;
}
```

<figure><img src="../../.gitbook/assets/image (4).png" alt=""><figcaption><p>When <code>Position</code> is selected, it uses the Vector3 <code>targetPosition</code></p></figcaption></figure>

<figure><img src="../../.gitbook/assets/image (5).png" alt=""><figcaption><p>When <code>Transform</code> is selected, uses the Transform <code>targetTransform</code></p></figcaption></figure>
